...
- **State assumptions explicitly.** If you are about to use Adam defaults `(β₁=0.9, β₂=0.999)` say so out loud — HINT++ is asymmetric `β₁ < β₂` by design (spec-fixed `β₁=0.7, β₂=0.95`, asserted in `__init__`). If you are unsure whether a tensor is on CPU or GPU, whether voxelized or raw, whether `xyz` or `xyz+rgb` — ask, don't guess.
...
- **Which structure are we in?** The CLAUDE.md directory map marks what EXISTS vs what is PLANNED — check it before scaffolding. Today only `src/safety/` exists under `src/`; `harness/` and `paper/` are planned. Don't import from a module that doesn't exist yet — scaffold it explicitly.
- **Which phase boundary?** Check the status snapshot in CLAUDE.md / `phase-implement` before assuming a phase is spec-only or done.
...
- **No "flexibility" or "configurability" that wasn't requested.** `configs/safety.yaml` is the canonical hyperparameter source (full Hydra wiring lands in Phase 6) — don't add a `mode` argument with seven enum values "in case we need it later."
...
| "Implement Phase 3" | "`PermissionField(nn.Module)` exists, P_raw ∈ (0,1) verified by property test, gradient flows, signed-w sign propagation test passes" |
...
- For Phase 7 / primary experiments: the R1 success criteria (memo §6) — lowest RER at matched click budget, CCC ≈ 0, empirical harmful rate ≤ declared α_risk, within 3 pts of oracle-tuned HINT++.
...
- **Bias correction × informed priors (λ-mixture rule).** Never combine `1/(1−βᵗ)` zero-init bias correction with a nonzero-initialized EMA — it inflates the prior ×19 at t=1 for β₂=0.95 (R1 flaw F3). Priors enter ONLY via the pseudo-count mixture `λₖ = n₀/(n₀+Nₖ)` over zero-init bias-corrected internals (memo §3). Hand-verify the t=1 worked check (w ≈ 0.204·η·ηₖ) and `t=10000` stability in unit tests.
- **Running max is forbidden.** `P_safe = max(P_safe, P_raw)` and the term "monotone safety check" are RETIRED (R1 flaw F2: a permission that can only loosen is liveness, not safety). Never implement, test, or cite them — gate state is hysteresis + risk monitor (memo §4).
- **Permission field saturation.** A naive sigmoid can produce gradients on the order of `1e-30`. If the gated LoRA adaptation isn't moving, check the `P_raw(x)` distribution — if it's piled at 0 or 1, the gating has saturated (inspect the α scale in `P_raw = σ(α·w)`).
...
- **Forbidden references.** Never reference the project's grant program, sponsor organizations, host city, or application context in any code, comment, docstring, or paper text. This includes commit messages. (CLAUDE.md Critical Rules — the literal terms are deliberately written nowhere in the repo.)
...
- **Use `safety-check`** after any change to `src/safety/` or to estimator / permission / gate / monitor code.
